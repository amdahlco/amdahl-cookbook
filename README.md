...
- **Evals** — grade a drafted message against how your customers actually talk: pass in the message, poll the run, read the scorecard (verdict + per-dimension grades, the customer quotes that support or contradict it, and a grounded rewrite). Or author your own eval. [Recipe](prompts/agent-platform/evals.md).
