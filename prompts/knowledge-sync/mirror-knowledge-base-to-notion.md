...
- Put "research -> save to KB" on a cadence with a **Routine** — a scheduled Search created with the `agents` MCP tool (a name, a prompt, a cron) — so the doc that mirrors to Notion is refreshed the same way every time. For a fully-typed recipe instead, author a Workflow (console + REST): [How to write an Amdahl blueprint](../blueprints/authoring-a-blueprint.md).
