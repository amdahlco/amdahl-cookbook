...
- [Grade cold outbound with web evidence](evals-external-evidence.md) — the external tier: `include_external` on `evals.run` searches the public web and grades market claims against `external`-tier quotes — the answer to the cold-prospect gap (no account-tier evidence exists for a company you never spoke to). Covers the scope gate (`external_search:execute`), the pin conflict, the licence (market claims, never "our customers say"), and the cap clamp.
