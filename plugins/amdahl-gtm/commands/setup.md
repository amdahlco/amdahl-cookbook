...
2. **Tenant readiness.** From `context` summary + `settings` + `authors`, report what's on file and what's thin:
   ...
   - Author voice profiles present? If none, note that `/amdahl-gtm:draft` will produce generic voice until one exists.
