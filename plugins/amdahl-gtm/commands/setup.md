...
1. **Connection + auth.** Make one cheap Amdahl call: `search` { action: query, mode: "fuzzy", query: "how many customer interactions do we have on file, and how many in the last 30 days" }.
   ...
   - If the session lists no `search` tool at all, the server is not connected — there is no alternate surface to fall back to. Every workspace has `search` / `agents` / `evals`.
...
2. **Tenant readiness.** From the search result, report what's on file and what's thin:
   ...
   - If volume looks healthy, run one more quick read — `search` { action: query, mode: "fuzzy", query: "which data sources have interactions, by source" } — so the user sees which connectors are actually flowing.
